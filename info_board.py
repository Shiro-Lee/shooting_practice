...
class InfoBoard:
    ...
    def __init__(self, settings, screen, stats, timer):
        ...
        self.bullet_rect.centerx, self.bullet_rect.top = self.settings.screen_width/10, settings.screen_height/15
    ...
    def prep_bullets(self):
        ...
        self.bullet_left_image = self.bullet_font.render(bullet_str, True, self.text_color, self.settings.bg_color)
        # 将剩余弹药数显示在弹药图案右方
    ...
    def prep_timer(self):
        ...
        self.timer_image = self.timer_font.render(time_str, True, self.text_color, self.settings.bg_color)
        # 将计时数显示在左下角
        self.timer_rect = self.timer_image.get_rect()
        self.timer_rect.centerx = self.settings.screen_width/10
